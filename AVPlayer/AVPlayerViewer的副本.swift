...
class AVPlayerViewer: UIView {
    ...
    weak var ijk_player: MyIJKPlayerView?
    ...
    */
    ...
    func deploy_timer() {
        ...
        timer.setEventHandler(handler: { [weak self] in
            if (self?.timer_can_update ?? 1000) > 0 {
                ...
                // MARK: ---
                if let ijk = self?.ijk_player {
                    if let duration = ijk.player?.duration {
                        if self?.progress_label_right.text != self?.av_time.format(duration) {
                            self?.progress_label_right.text = self?.av_time.format(duration)
                        }
                    }
                    if let ijk_time = ijk.player?.currentPlaybackTime {
                        if self?.progress_label_left.text != self?.av_time.format(ijk_time) {
                            self?.progress_label_left.text = self?.av_time.format(ijk_time)
                        }
                    }
                }
                else
    ...
    @objc func play_aciton() {
        if let ijk = ijk_player {
            if ijk.player?.isPlaying() == true {
                ijk.player?.pause()
